...
def layer_perceptron(alfa):
    ...
    random_weights = []
    weights = {}
    for i in range(0, 27):  # dolaczamy prog, dlatego wektor jest 27 wymiarowy
    ...
    for language in os.listdir("data_training"):
        weights[language] = calculate_weights(training_data_vectors, alfa, language,
                                              random_weights)  ## mamy policzone wagi
...
def classify_input_text(weights):
    ...
    text = []
    print("Enter/Paste your text. Input ' :q ' to finish the current session of input.")
    while True:
        line = input()
        if line.strip() == ":q":
...
while True:
    ...
    command = input("Input ' :q ' for exit.")
    if command == ":q":
        break
